...
```{r raw-data-rmd, code=readLines(here::here("data-raw/NLSY79/NLSY79.R"))}
```
...
```
```{r dob-conflict-no, echo=FALSE, eval=!has_dob_conflict}
cat("All birth month and year recorded in 1979 and 1981 match.")
```
```{r dob-conflict-yes, echo=has_dob_conflict, eval=has_dob_conflict}
